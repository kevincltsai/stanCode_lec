...
class TwoLayerNet(object):
    ...
    def cost(self, X, y=None):
        ...
        """
        # Unpack variables from th     e params dictionary
        ...
        #K = np.dot(W1.T, X.T) + b1.reshape((b1.shape[0],1))
        K = np.dot(X, W1) + b1.reshape(1, W1.shape[1])
        A = np.maximum(0, K)
        #scores = np.dot(W2.T, A) + b2.reshape((b2.shape[0],1))
        scores = np.dot(A, W2) + b2.reshape((1, W2.shape[1]))
        scores = scores
        ...
        #print(grads_classScores.shape)
        grads['b2'] = np.sum(grads_classScores, axis = 0)
        grads['W2'] = np.dot(A.T, grads_classScores)
       
        da = np.dot(grads_classScores, W2.T)
        
        dk = da * np.where(K>0, 1, 0)
        grads['W1'] = np.dot(X.T, dk)
        grads['b1'] = np.sum(dk, axis=0, keepdims=True) 
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            # TODO: Create a random minib      atch of training data and labels, storing  #
            ...
            rnd = random.choices(range(0, X.shape[0]),k=batch_size) 
            X_batch = X[rnd]
            y_batch = y[rnd]
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    def predict(self, X):
        ...
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
               #K = np.dot(W1.T, X.T) + b1.reshape((b1.shape[0],1))
        K = np.dot(X, self.params['W1']) + self.params['b1'].reshape(1, self.params['W1'].shape[1])
        A = np.maximum(0, K)  
        scores = np.dot(A, self.params['W2']) + self.params['b2'].reshape((1, self.params['W2'].shape[1]))
        scores -= np.amax(scores, axis=1,keepdims=True)
