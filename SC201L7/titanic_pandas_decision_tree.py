...
def main():
    ...
    d_tree_classfier = d_tree.fit(x_train,y)
    train_acc = d_tree_classfier.score(x_train,y)
    print('Train acc:', train_acc)
    tree.export_graphviz(d_tree_classfier, feature_names=feature_names, out_file='tree')
    #tree.plot_tree(d_tree_classfier)

    test_data = data_preprocess(TEST_FILE, mode = 'Test')
    x_test = test_data[feature_names]
    predictions = d_tree_classfier.predict(x_test)
    out_file(predictions, 'd_reg_tree.csv')
