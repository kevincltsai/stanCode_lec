...
#NUM_EPOCHS = 100000                   # This constant controls the total number of epochs
...
def normalize_all(arr):
    ...
    :return: numpy_array, the values are normalized, where the dimension is still (num_phi, m)
    """
    min_array = np.min(arr, axis=1, keepdims=True)
    max_array = np.max(arr, axis=1, keepdims=True)
    return (arr - min_array) / (max_array - min_array)
...
def neural_network(X, Y):
    ...
    for i in range(1, L + 1): #{1,2,3,4,5,6}
        ...
        weights['W'+str(i)] = np.random.randn(NODES['N' + str(i-1)], NODES['N'+str(i)]) * np.sqrt(2/NODES['N' + str(i-1)])
        biases['B'+str(i)] = np.random.randn(NODES['N'+str(i)],1) * np.sqrt(2/NODES['N' + str(i-1)])
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i in range(1, L + 1):
            ...
            dw_hist[i-1].append(np.nanmean(ALPHA * dW_lst['dW' + str(i)]))
            
        
    df = pd.DataFrame(dw_hist)
    df=df.transpose()
